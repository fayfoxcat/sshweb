...
{:else}
  ...
  >
    ...
    >
      ...
      {:else if $authStatus.authenticated && $authStatus.pendingChange}
        <!-- Forced first-password change after a setup-key login -->
        ...
        >
          ...
          <button class="btn-primary" type="submit" disabled={busy}>
            {busy ? t($lang, "auth.processing") : t($lang, "auth.forceBtn")}
          </button>
        </form>
      {:else}
        ...
        >
          ...
          <label class="field">
            ...
            <input
              class="input-auth"
              type="password"
              autocomplete={$authStatus.setup ? "current-password" : "off"}
              bind:value={password}
            />
          </label>
          {#if error}
