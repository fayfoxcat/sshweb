<script lang="ts">
  ...
  interface Props {
    ...
    /** Called to re-read the file from disk. */
    onReload?: () => void;
  }
  ...
  let encoding = $state<Encoding>("utf-8");
  let encodingMenu = $state(false);
  ...
  let winW = $state(Math.round(window.innerWidth * 0.8));
  let winH = $state(Math.round(window.innerHeight * 0.8));
  let posX = $state(0);
  let posY = $state(0);
  let dragStartMX = 0;
  let dragStartMY = 0;
  ...
  let pendingBytes = $state<Uint8Array | null>(null);
  ...
  /** Overlay an unsaved draft (if any) on freshly-loaded disk content. */
  ...
  /** Discard unsaved edits and re-read the file from disk. */
  function doRevert() {
    clearDraft();
    savePending = false;
    saveText = "";
   ...
   *  server acknowledges a save (`sftpOk`). */
  ...
  /** File size in human form; `-` until the content bytes have arrived. */
  let fileSizeText = $derived(
    pendingBytes ? formatSize(pendingBytes.length) : "-",
  );
</script>
...
>
  ...
  >
    ...
    <div
      ...
      onpointerdown={winDrag.onStart}
      onpointermove={winDrag.onMove}
      onpointerup={winDrag.onEnd}
      onpointercancel={winDrag.onEnd}
    >
      <button
        class="icon-btn"
        onclick={close}
      ...
      <div class="relative">
        <button
          ...
          onclick={() => (encodingMenu = !encodingMenu)}
          onpointerdown={(e) => e.stopPropagation()}
    ...
    <div
      ...
      onpointerdown={resizeDrag.onStart}
      onpointermove={resizeDrag.onMove}
      onpointerup={resizeDrag.onEnd}
      onpointercancel={resizeDrag.onEnd}
    ></div>
  </div>
</div>
