import { writable } from "svelte/store";

import { request } from "./api";
import type { WsServerConfig, WsSocks5Tunnel } from "./protocol";

/** 「本机」直连代理在服务端注册表里的键（与 `serverTargetKey(null)` 一致）。 */
export const LOCAL_PROXY_KEY = "local";

/** 一个运行中的 SOCKS5 代理（服务端状态）。 */
export type ProxyStatus = {
  /** 服务器标识 `user@host:port`（与 `serverTargetKey` 一致），本机为 `local`。 */
  serverKey: string;
  /** 显示名。本机的值来自服务端字面量，界面要显示时用 `t($lang,"servers.local")`。 */
  name: string;
  /** 本地监听端口。 */
  port: number;
};

const proxyStore = writable<ProxyStatus[]>([]);

/** 当前运行中的 SOCKS5 代理（serverKey → 状态）。 */
export const proxies = proxyStore;

/** 刷新代理列表（服务器面板打开 / 登录后调用）。 */
export async function loadProxies(): Promise<void> {
  const list = await request<ProxyStatus[]>("/api/proxies");
  proxyStore.set(list);
}

/** 开启某服务器的 SOCKS5 隧道。`port` 为 0 时由服务端自动分配。 */
export async function startProxy(
  server: WsServerConfig,
  port = 0,
): Promise<ProxyStatus> {
  const status = await request<ProxyStatus>("/api/proxies", {
    method: "POST",
    body: JSON.stringify({ server, port }),
  });
  proxyStore.update((list) => [
    ...list.filter((p) => p.serverKey !== status.serverKey),
    status,
  ]);
  return status;
}

/** 开启「本机」直连 SOCKS5 代理（不走 SSH，端口只绑 127.0.0.1）。
 *  走专用端点：本机没有 `ServerConfig`，而把 `/api/proxies` 的 `server` 改成
 *  可选会让「客户端漏字段」变成静默启动一个 no-auth 直连代理（已知坑 81/83）。 */
export async function startLocalProxy(
  tunnel?: WsSocks5Tunnel,
): Promise<ProxyStatus> {
  const status = await request<ProxyStatus>("/api/proxies/local", {
    method: "POST",
    body: JSON.stringify({
      port: tunnel?.port ?? 0,
      username: tunnel?.username ?? "",
      password: tunnel?.password ?? "",
    }),
  });
  proxyStore.update((list) => [
    ...list.filter((p) => p.serverKey !== status.serverKey),
    status,
  ]);
  return status;
}

/** 停止一条 SOCKS5 代理（远程隧道与本机直连共用，按 key）。 */
export async function stopProxy(serverKey: string): Promise<void> {
  await request<void>(`/api/proxies/${encodeURIComponent(serverKey)}`, {
    method: "DELETE",
  });
  proxyStore.update((list) => list.filter((p) => p.serverKey !== serverKey));
}
